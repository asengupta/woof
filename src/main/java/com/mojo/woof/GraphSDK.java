...
public class GraphSDK implements AutoCloseable {
  ...
  public ActionResult traverse(Record node, NodeAction nodeAction, String parentChildRelationship) {
    ...
    List<ActionResult> childResults =
        children.stream().map(n -> traverse(n, nodeAction, parentChildRelationship)).toList();
    ...
    try (Session session = driver.session(builder.sessionConfig())) {
      return session.executeRead(
          tx -> {
            Query query =
                new Query(
                    String.format("MATCH (start {id: $rootID})-[:%s]->(n)", parentChildRelationship)
                        + "            RETURN DISTINCT n",
                    parameters("rootID", id(node)));
            Result result = tx.run(query);
            return result.list();
          });
    }
              ...
              tx -> {
                Query query =
                    new Query(
                        "MATCH (p {id: $parentId}) "
                            + String.format(
                                "CREATE (n:%s {id: $childId, type: 'SUMMARY', text: $text}) ",
                                SUMMARY_NODE)
    ...
    try (Session session = driver.session(builder.sessionConfig())) {
      Record record =
          session.executeWrite(
              tx -> {
                return tx.run(
                        new Query("MATCH (n {id: $id}) RETURN n", Values.parameters("id", id)))
                    .single();
              });
      return record;
    }
                ...
                Query query =
                    new Query(
                        "MATCH (p {id: $parentId}) "
                            + " MATCH (c {id: $childId}) "
                            + String.format(
                                "MERGE (p)-[r:%s {edgeType: $edgeType}]->(c) ", relationshipName)
                            + "RETURN p, c, r",
                        parameters(
                            "parentId", id(parent), "childId", id(child), EDGE_TYPE, edgeType));
                return tx.run(query).single();
              });
      return record;
    }
      ...
      List<Record> records =
          session.executeWrite(
              tx -> {
                ...
                List<String> searchSpecs =
                    propertySpec.entrySet().stream()
                        .map(e -> e.getKey() + ": $" + e.getKey())
                        .toList();
                String propertySearchSpec = String.join(",", searchSpecs);
                String queryString =
                    String.format("MATCH (%s {%s}) ", nodeIdentifier, propertySearchSpec)
                        + " RETURN n";
  ...
  public Record newOrExisting(
      List<String> labels, Map<String, Object> propertySpec, WoofNode newNode) {
